...
const alertVariants = cva('flex items-stretch w-full gap-2 group-[.toaster]:w-(--width)', {
  variants: {
    ...
    size: {
      ...
      md: 'rounded-md p-2.5 gap-2.5 text-sm [&>[data-slot=alert-icon]>svg]:size-5 *:data-slot=alert-icon:mt-0 [&_[data-slot=alert-close]]:mt-0.5',
...
function AlertTitle({ className, ...props }: React.HTMLAttributes<HTMLHeadingElement>) {
  return <div data-slot="alert-title" className={cn('text-xs grow ', className)} {...props} />;
}
...
function AlertDescription({ className, ...props }: React.HTMLAttributes<HTMLParagraphElement>) {
  return (
    <div
      ...
      className={cn('text-xs [&_p]:leading-relaxed [&_p]:mb-2', className)}
      {...props}
    />
  );
}
...
function AlertContent({ className, ...props }: React.HTMLAttributes<HTMLParagraphElement>) {
  return (
    <div
      ...
      className={cn('[&_[data-slot=alert-title]]:font-semibold', className)}
      {...props}
    />
  );
}
