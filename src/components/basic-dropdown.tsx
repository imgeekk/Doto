    ...
    <AnimatePresence>
      {open && (
        <motion.div
          ...
          className={`absolute z-[9999] min-w-[8rem] overflow-hidden rounded-md border border-border bg-card text-card-foreground p-1 shadow-md dark:shadow-[0_2px_10px_rgba(0,0,0,0.3)] ${positionClasses} ${alignmentClasses} ${className}`}
          role="menu"
  ...
  return (
    <button
      className={`relative flex w-full cursor-pointer select-none items-center rounded-sm px-2 py-2 text-sm outline-none transition-colors ${
        disabled
          ? "pointer-events-none opacity-100 text-muted-foreground"
          :   destructive
